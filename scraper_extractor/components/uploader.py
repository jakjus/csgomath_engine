...
class Uploader:
    ...
    def insert(self):
        ...
        for case in self.extracted_data:
            self.cursor.execute("INSERT IGNORE INTO cases (name, icon_url) VALUES (%s, %s)", (case['name'], case['asset_description']['icon_url']))
            ...
            for i, descField in enumerate(case['asset_description']['descriptions']):
                ...
                ins0 = "INSERT INTO descriptionFields (caseId, ind, value, color) VALUES (%s, %s, %s, %s) ON DUPLICATE KEY UPDATE value=value, color=color"
            ...
            if 'key' in case.keys():
                for i, descField in enumerate(case['key']['asset_description']['descriptions']):
                    ...
                    if 'color' not in descField.keys():
                        descField['color']='NULL'
                    ins0 = "INSERT INTO keysDescriptionFields (keyId, ind, value, color) VALUES (%s, %s, %s, %s) ON DUPLICATE KEY UPDATE value=value, color=color"
